...
def innerL(i, oS):
    ...
    if ((oS.labelMat[i] * Ei < -oS.tol) and (oS.alphas[i] < oS.C)) or (
        ...
        updateEk(oS, i)
        # 步骤7：更新b_1和b_2:
        b1 = oS.b - Ei - oS.labelMat[i] * (oS.alphas[i] - alpha_i_old) * oS.X[i, :] * oS.X[i, :].T - oS.labelMat[j] * (
                    oS.alphas[j] - alpha_j_old) * oS.X[j, :] * oS.X[i, :].T
        b2 = oS.b - Ej - oS.labelMat[i] * (oS.alphas[i] - alpha_i_old) * oS.X[i, :] * oS.X[j, :].T - oS.labelMat[j] * (
...
def showClassifer(dataMat, classLabels, w, b):
    ...
    plt.title('完整版SMO得到的支持向量')
    # plt.title('完整版SMO得到的支持向量',fontproperties = myfont)
    # 找出支持向量点
